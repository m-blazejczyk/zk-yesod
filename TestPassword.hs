...
pbkdf1 password (SaltBS salt) iter = hashRounds first_hash (iter + 1)
    where first_hash = SHA.finalize $ SHA.init `SHA.update` password `SHA.update` salt
...
hashRounds (!bs) 0 = bs
hashRounds bs rounds = hashRounds (SHA.hash bs) (rounds - 1)
...
makePasswordSaltWith algorithm strengthModifier pwd salt strength = writePwHash (strength, salt, hash)
    where hash = B64.encode $ algorithm pwd salt (strengthModifier strength)
...
verifyPasswordWith algorithm strengthModifier userInput pwHash =
    case readPwHash pwHash of
      ...
      Just (strength, salt, goodHash) ->
          B64.encode (algorithm userInput salt (strengthModifier strength)) == goodHash
    ...
    case readPwHash pwHash of
      ...
      Just (oldstr, salt, hashB64) ->
          if oldstr < newstr then
              ...
              pwHash
          where newHash = B64.encode $ hashRounds hash extraRounds
                extraRounds = (2^newstr) - (2^oldstr)
                hash = B64.decodeLenient hashB64
...
makeSalt = SaltBS . B64.encode . check_length
...
main = do
    saltedPwd <- saltPass "abcd"
    putStrLn $ TS.unpack $ saltedPwd
    putStrLn $ show $ isValidPass "abcd" saltedPwd
